...
extension UIImage{
    ...
    func trimmed(image : UIImage) -> UIImage{
        ...
        var bottom : CGPoint?
        var breakOut = false
        for var x = 0; breakOut == false && x < width; x += 1{
            for y in 0 ..< height{
                var loc = x + (y * width)
                loc *= 4
        ...
        breakOut = false
        for var y = 0; breakOut == false && y < height; y += 1{
            for x in 0 ..< width{
                var loc = x + (y * width)
                loc *= 4
                if m_PixelBuf[loc + 3] != 0{
        ...
        breakOut = false
        for var y = height - 1; breakOut == false && y >= 0; y -= 1{
        ...
        breakOut = false
        for var x = width - 1; breakOut == false && x >= 0; x -= 1{
